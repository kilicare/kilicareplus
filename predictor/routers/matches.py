...
CODES    = {"EPL":"PL", "LA_LIGA":"PD", "BUNDESLIGA":"BL1"}
...
def _fetch(endpoint, params=None):
    ...
    try:
        ...
        if r.status_code == 429:
            time.sleep(65); return _fetch(endpoint, params)
        return None
    except: return None
...
def _sample(league):
    SAMPLES = {
        'EPL': [('Manchester City FC','Arsenal FC'),('Liverpool FC','Chelsea FC'),
                ('Tottenham Hotspur FC','Manchester United FC'),('Newcastle United FC','Aston Villa FC')],
        'LA_LIGA': [('Real Madrid CF','FC Barcelona'),('Atletico de Madrid','Real Sociedad de Fútbol'),
                    ('Villarreal CF','Athletic Club')],
        'BUNDESLIGA': [('FC Bayern München','Borussia Dortmund'),('RB Leipzig','Bayer 04 Leverkusen'),
                       ('Eintracht Frankfurt','VfL Wolfsburg')],
    }
    base = datetime.now()
    return {"league": league, "source": "sample", "matches": [
        {"external_id": f"sample_{league}_{i}", "league": league,
         "home_team": h, "away_team": a,
         "home_team_logo": None, "away_team_logo": None,
         "scheduled_at": (base + timedelta(days=i*2+1)).isoformat(),
         "matchday": 20, "status": "SCHEDULED"}
        for i,(h,a) in enumerate(SAMPLES.get(league,[]))
    ], "count": len(SAMPLES.get(league,[]))}
...
def get_upcoming(league: str = Query("EPL")):
    code = CODES.get(league)
    if not code: raise HTTPException(400, f"Unknown league: {league}")
    if not API_KEY: return _sample(league)
    data = _fetch(f"competitions/{code}/matches", {"status":"SCHEDULED","limit":10})
    if not data: return _sample(league)
    matches = []
    for m in data.get("matches",[])[:10]:
        matches.append({"external_id":str(m.get("id")), "league":league,
                        "home_team":m["homeTeam"]["name"],
                        "away_team":m["awayTeam"]["name"],
                        "home_team_logo":m["homeTeam"].get("crest"),
                        "away_team_logo":m["awayTeam"].get("crest"),
                        "scheduled_at":m.get("utcDate"),
                        "matchday":m.get("matchday",1), "status":m.get("status")})
...
def get_standings(league: str = Query("EPL")):
    code = CODES.get(league)
    if not code: raise HTTPException(400, f"Unknown league: {league}")
    if not API_KEY: return {"message":"API key required"}
    data = _fetch(f"competitions/{code}/standings")
    if not data: return {"message":"Could not fetch"}
    standings = []
    try:
        for e in data["standings"][0]["table"][:20]:
            standings.append({"position":e["position"],"team":e["team"]["name"],
                               "played":e["playedGames"],"won":e["won"],
                               "drawn":e["draw"],"lost":e["lost"],
                               "goals_for":e["goalsFor"],"goals_against":e["goalsAgainst"],
                               "goal_diff":e["goalDifference"],"points":e["points"]})
    except: pass
